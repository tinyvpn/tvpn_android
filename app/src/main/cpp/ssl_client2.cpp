...
int init_ssl_client( ) {
     ...
     */
    __android_log_print(ANDROID_LOG_DEBUG, "JNI", "  . Seeding the random number generator...");
    fflush(stdout);
...
//    __android_log_print(ANDROID_LOG_DEBUG, "JNI", " ok");
    /*
     * 1.1. Load the trusted CA
     */
    __android_log_print(ANDROID_LOG_DEBUG, "JNI", "  . Loading the CA root certificate ...");
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "JNI", " ok (%d skipped)", ret);
     ...
     */
    __android_log_print(ANDROID_LOG_DEBUG, "JNI", "  . Loading the client cert. and key...");
    ...
    __android_log_print(ANDROID_LOG_INFO, "JNI", "ssl init ok");
...
int connect_ssl(std::string& ip, uint16_t port, int& sock) {
    ...
    __android_log_print(ANDROID_LOG_INFO, "JNI", "  . Connecting to %s/%s/%s...",
     ...
     * 3. Setup stuff
     */
    __android_log_print(ANDROID_LOG_INFO, "JNI", "  . Setting up the SSL/TLS structure...");
    ...
    if ((ret = mbedtls_ssl_conf_max_frag_len(&conf, opt.mfl_code)) != 0) {
        __android_log_print(ANDROID_LOG_ERROR, "JNI", " failed! mbedtls_ssl_conf_max_frag_len returned %d", ret);
        return 1;
    }
   ...
   // mbedtls_ssl_set_bio(&ssl, &server_fd, mbedtls_net_send, NULL, mbedtls_net_recv_timeout);
     ...
     * 4. Handshake
     */
    __android_log_print(ANDROID_LOG_INFO, "JNI", "  . Performing the SSL/TLS handshake...");

    while ((ret = mbedtls_ssl_handshake(&ssl)) != 0) {
        if (ret != MBEDTLS_ERR_SSL_WANT_READ &&
            ret != MBEDTLS_ERR_SSL_WANT_WRITE &&
            ret != MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS) {
            __android_log_print(ANDROID_LOG_ERROR, "JNI", " failed! mbedtls_ssl_handshake returned -0x%x", -ret);
    ...
    __android_log_print(ANDROID_LOG_INFO, "JNI", " ok. [ Protocol is %s ][ Ciphersuite is %s ]",
                   mbedtls_ssl_get_version(&ssl),
                   mbedtls_ssl_get_ciphersuite(&ssl));

    if ((ret = mbedtls_ssl_get_record_expansion(&ssl)) >= 0)
        __android_log_print(ANDROID_LOG_INFO, "JNI", "    [ Record expansion is %d ]", ret);
    else
        __android_log_print(ANDROID_LOG_ERROR, "JNI", "    [ Record expansion is unknown (compression) ]");

    __android_log_print(ANDROID_LOG_INFO, "JNI", "    [ Maximum fragment length is %u ]",
                   (unsigned int) mbedtls_ssl_get_max_frag_len(&ssl));
    /*
     * 5. Verify the server certificate
     */
    __android_log_print(ANDROID_LOG_INFO, "JNI", "  . Verifying peer X.509 certificate...");

    if ((flags = mbedtls_ssl_get_verify_result(&ssl)) != 0) {
        char vrfy_buf[512];
//        __android_log_print(ANDROID_LOG_ERROR, "JNI", " failed\n");
        mbedtls_x509_crt_verify_info(vrfy_buf, sizeof(vrfy_buf), "  ! ", flags);
        __android_log_print(ANDROID_LOG_ERROR, "JNI", "failed: %s", vrfy_buf);
    }

    if (mbedtls_ssl_get_peer_cert(&ssl) != NULL) {
        mbedtls_x509_crt_info((char *) buf, sizeof(buf) - 1, "      ",
                              mbedtls_ssl_get_peer_cert(&ssl));
        __android_log_print(ANDROID_LOG_INFO, "JNI", "  . Peer certificate information:%s", buf);
    }
    return 0;
}
int get_private_ip(int premium, std::string& androidId, std::string& userName, std::string& userPassword, std::string& recv_data) {
        ...
        do {
            while ((ret = mbedtls_ssl_write(&ssl, buf + written, len - written)) < 0) {
                if (ret != MBEDTLS_ERR_SSL_WANT_READ &&
                    ret != MBEDTLS_ERR_SSL_WANT_WRITE &&
                    ret != MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS) {
                    __android_log_print(ANDROID_LOG_ERROR, "JNI", " failed! mbedtls_ssl_write returned -0x%x", -ret);
                    return 1;
                }
            }
            frags++;
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "JNI", " %d bytes written in %d fragments", written, frags);
        ...
        if (ret <= 0) {
            switch (ret) {
                case MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY:
                    __android_log_print(ANDROID_LOG_ERROR, "JNI", " connection was closed gracefully");
                    return 1;
                case 0:
                case MBEDTLS_ERR_NET_CONN_RESET:
                    __android_log_print(ANDROID_LOG_ERROR, "JNI", " connection was reset by peer");
                    return 1;
                default:
                    __android_log_print(ANDROID_LOG_ERROR, "JNI", " mbedtls_ssl_read returned -0x%x", -ret);
                    return 1;
            }
        }

        len = ret;
        __android_log_print(ANDROID_LOG_DEBUG, "JNI", " %d bytes read", len);
...
int ssl_read(char* ip_packet_data, int& ip_packet_len)
{
    ...
    //__android_log_print(ANDROID_LOG_DEBUG, "JNI", " %d bytes read", ret);
    /* End of message should be detected according to the syntax of the
     * application protocol (eg HTTP), just use a dummy test here. */
    if( ret > 0  )
    {
    ...
    do
    {
        while( ( ret = mbedtls_ssl_write( &ssl, buf + written, len - written ) ) < 0 )
        {
            if( ret != MBEDTLS_ERR_SSL_WANT_READ &&
                ...
                ret != MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS )
            {
                __android_log_print(ANDROID_LOG_ERROR, "JNI", " failed! mbedtls_ssl_write returned -0x%x,%d,%d,%d", -ret ,len,written,frags);
                return 1;
            }
        }
        frags++;
